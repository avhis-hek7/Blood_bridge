...
//     if (!user || !participatedEvent) return;

//     try {
//       await axios.post("http://localhost:5000/api/participation/cancel", {
//         email: user.email,
//         eventTitle: participatedEvent.title
//       });

//       setHasParticipated(false);
//       setParticipatedEvent(null);
//       setStatusMessage("✅ Participation cancelled successfully.");
//       hideStatusAfterDelay();
//     } catch (err) {
//       console.error("Error cancelling participation:", err);
//       setStatusMessage("❌ Failed to cancel participation. Please try again.");
//       hideStatusAfterDelay();
//     }
//   };
        ...
        <div className="d-flex gap-2">
          {isLoggedIn && (
            <>
              ...
              {hasParticipated &&
                participatedEvent &&
                getEventStatus(participatedEvent).status === "Upcoming" && (
                  <button
                    className="btn btn-danger pulse-on-hover"
                    onClick={handleCancelParticipation}
                  >
